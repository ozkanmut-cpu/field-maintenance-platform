...
const baseline:any = { technicianId:'t1', state:'ACTIVE', confidence:'MEDIUM', observedWeeks:8, serviceEvidenceWeeks:8, travelEvidenceWeeks:6, service:{ completedVisits:{median:8,p75:10,p90:12}, coolerCount:{median:20,p75:25,p90:30}, towerCount:{median:8,p75:10,p90:12}, tapCount:{median:6,p75:8,p90:10}, smarttapCount:{median:2,p75:3,p90:4}, estimatedServiceEffortMidpointMinutes:{median:180,p75:225,p90:270} }, travel:{ routeDistanceMeters:{median:10000,p75:15000,p90:20000}, fieldP90RadiusMeters:{median:5000,p75:7000,p90:9000}, routeCoherenceRatio:{median:1.2,p75:1.5,p90:2}, fragmentationRatio:{median:0.2,p75:0.4,p90:0.6} }, context:{uniqueVisitedPoints:{median:8,p75:10,p90:12}}, reasons:[] };
const assigned:any = { technicianId:'t1', standardCurrent:4, standardCarryover:0, smartcleanCurrent:1, smartcleanCarryover:0, equipmentKnownPointCount:5, equipmentUnknownPointCount:0, assignedCoolerCount:20, assignedTowerCount:8, assignedTapCount:6, assignedSmarttapCount:2, assignedServiceEffortMinMinutes:120, assignedServiceEffortMaxMinutes:240, assignedServiceEffortMidpointMinutes:180, assignedLocatedPointCount:5, assignedUnlocatedPointCount:0, assignedFieldP90RadiusMeters:5000, assignedRouteEstimateMeters:12000, assignedRouteCoherenceRatio:1.2, assignedClusterCount:1, assignedIsolatedPointCount:0, assignedFragmentationRatio:0.2, workAreaCenterDistanceMeters:1000 };
...
test('equipment increase changes service pressure and risk without mutating source', () => {
  ...
  assert.equal(result.after.assigned.assignedServiceEffortMidpointMinutes, 180);
});

test('tower delta recalculates normative service effort without measuring duration', () => {
  const result = service.simulate(assigned, baseline, maturity, { towerDelta: 2 });
  assert.equal(result.after.assigned.assignedTowerCount, 10);
  assert.equal(result.after.assigned.assignedServiceEffortMinMinutes, 150);
  assert.equal(result.after.assigned.assignedServiceEffortMaxMinutes, 300);
  assert.equal(assigned.assignedTowerCount, 8);
});
