...
test('equipment increase changes service pressure and risk without mutating source', () => {
  const result = service.simulate(assigned, baseline, maturity, { coolerDelta: 15 }, undefined, calibration);
...
test('equipment delta recalculates learned service workload index without inventing minutes', () => {
  const result = service.simulate(assigned, baseline, maturity, { towerDelta: 2 }, undefined, calibration);
  assert.equal(result.after.assigned.assignedTowerCount, 10);
  assert.equal(result.before.workload.serviceWorkload.index, 100);
  assert.ok((result.after.workload.serviceWorkload.index ?? 0) > 100);
  assert.equal(assigned.assignedTowerCount, 8);
});

test('route what-if can expose travel risk delta', () => {
  const result = service.simulate(assigned, baseline, maturity, { routeEstimateMeters: 25000 }, undefined, calibration);
...
test('point removal style negative deltas are clamped safely at zero', () => {
  const result = service.simulate(assigned, baseline, maturity, { standardCurrentDelta: -99, coolerDelta: -99 }, undefined, calibration);
...
test('technician placement comparison prefers the lower post-change risk and never mutates assignments', () => {
  ...
  const result = service.comparePlacement(assigned, baseline, target, targetBaseline, maturity, { standardCurrentDelta:6, coolerDelta:20 }, undefined, calibration);
...
test('region placement applies the real region workload vector without changing assignment state', () => {
  ...
  const result = service.simulateRegionPlacement(region, 't1', target, targetBaseline, maturity, undefined, calibration);
