...
function input(overrides: any = {}): PlanningTechnicianInput {
  return {
    ...
    assigned: {
      ...
      assignedFieldP90RadiusMeters: 5000, assignedRouteEstimateMeters: 12000,
      ...overrides.assigned,
    },
    workload: {
      ...
      travelPressure: { routeDistanceMeters: 'WITHIN_BASELINE', fieldP90RadiusMeters: 'WITHIN_BASELINE' }, reasons: [],
      ...overrides.workload,
    },
...
test('route pressure produces route review while incomplete profiles produce data-quality review', () => {
  const result = service.assess([input({ assigned: { equipmentUnknownPointCount: 1 }, workload: { travelPressure: { routeDistanceMeters: 'ABOVE_P90', fieldP90RadiusMeters: 'WITHIN_BASELINE' } } })], maturity);
