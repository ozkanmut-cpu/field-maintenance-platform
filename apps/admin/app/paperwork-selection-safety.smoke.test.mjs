...
test('bulk paperwork actions exclude selections hidden by search', () => {
  ...
  assert.match(source, /onChange=\{\(e\) => \{ setSelected\(\[\]\); setTechnicianId\(e\.target\.value\); \}\}/,
    'a technician change must synchronously clear prior selections');
  assert.match(source, /onChange=\{\(e\) => \{ setSelected\(\[\]\); setDate\(e\.target\.value\); \}\}/,
    'a date change must synchronously clear prior selections');
