...
export default function Home() {
  ...
  return (
    <AdminShell me={me} location={location} onNavigate={navigate} onLogout={() => void signOut()}>
      ...
      {section === 'location-matching' ? <LocationMatching /> : section === 'ai-dashboard' ? <AiDashboard /> : section === 'anomalies' ? <AnomalyReview /> : section === 'maintenance-calendar' ? <MaintenanceCalendar /> : section === 'prospects' ? <Prospects /> : section === 'audit-log' ? <AuditLog /> : section === 'point-detail' ? <PointDetails /> : section === 'assignments' ? <AssignmentManagement /> : section === 'paperwork' ? <PaperworkManagement /> : section === 'point-timeline' ? <PointTimeline /> : section === 'duplicates' ? <DuplicateSuggestions /> : section === 'non-maintenance-visits' ? <NonMaintenanceVisits /> : section === 'sap-sync' ? <SapSyncStatus /> : section === 'dashboard' || section === 'approvals' || section === 'setup-pending' || section === 'regions' || section === 'points' ? <Operations users={users} activeSection={section} onNavigate={navigate} /> : null}
