...
export default function PointDetailPage({ location, onNavigate: navigate }: Props) {
  ...
  const header = <section className="pointIdentity" aria-label="Nokta kimliği ve durum">
    ...
    <div className="pointIdentityFacts"><span className={point.status === 'ACTIVE' ? 'pill active' : 'pill'}>{point.status === 'ACTIVE' ? 'Aktif' : point.status === 'PASSIVE' ? 'Pasif' : 'İptal'}</span><span><strong>Bakım tipi:</strong> {point.maintenanceType}</span><span><strong>Geçerli teknisyen:</strong> <span>{assignmentError ? <span role="alert">Geçerli teknisyen yüklenemedi · {assignmentError} <button className="ghost" onClick={() => setAssignmentRetry(value => value + 1)}>Geçerli teknisyeni yeniden dene</button></span> : effectiveAssignment ? effectiveAssignment.technician?.name ?? 'Atanmamış' : 'Yükleniyor…'}</span></span><span><strong>Konum:</strong> {point.locationSource ?? 'UNKNOWN'} / {point.locationConfidence === null || point.locationConfidence === undefined ? '—' : `${point.locationConfidence}%`}</span></div>
