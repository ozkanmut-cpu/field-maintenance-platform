...
export default function PointDetailPage({ location, onNavigate }: Props) {
  ...
  useEffect(() => {
    ...
    if (!endpoint) { setTabItems([]); return; }
    let cancelled = false; setTabLoading(true); setTabItems([]);
    void fetch(endpoint).then(async (response) => { if (!response.ok) throw new Error(`HTTP ${response.status}`); return response.json(); }).then((data) => { if (!cancelled) setTabItems(asItems(data)); }).catch((cause) => { if (!cancelled) setError(cause instanceof Error ? cause.message : String(cause)); }).finally(() => { if (!cancelled) setTabLoading(false); });
