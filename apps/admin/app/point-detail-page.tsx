...
export default function PointDetailPage({ location, onNavigate: navigate }: Props) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/backend/maintenance/point-timeline?pointId=${encodeURIComponent(location.pointId)}`)
      ...
      .then(data => { if (!cancelled) setRecentActivity(asItems(data).filter(item => typeof item.at === 'string' && !Number.isNaN(Date.parse(item.at)) && Date.parse(item.at) <= Date.now()).sort((a, b) => Date.parse(String(b.at)) - Date.parse(String(a.at))).slice(0, 20)); })
  ...
  useEffect(() => { if (!pointId) return; let cancelled = false; setEffectiveAssignment(null); setAssignmentError(''); void fetch(`/api/backend/assignments/effective/${pointId}`).then(async (response) => { if (!response.ok) throw new Error(`HTTP ${response.status}`); return response.json() as Promise<EffectiveAssignment>; }).then((assignment) => { if (!cancelled) setEffectiveAssignment(assignment); }).catch((cause) => { if (!cancelled) setAssignmentError(cause instanceof Error ? cause.message : String(cause)); }); return () => { cancelled = true; }; }, [pointId, assignmentRetry]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    let cancelled = false; setTabLoading(true); setTabError(''); setTabItems([]); setMaintenanceTotals(null);
    void fetch(endpoint).then(async (response) => { if (!response.ok) throw new Error(`HTTP ${response.status}`); return response.json(); }).then((data) => { if (!cancelled) { setTabItems(asItems(data)); if (activeTab === 'maintenance' && data && typeof data === 'object' && 'totals' in data && (data as Record<string, unknown>).totals && typeof (data as Record<string, unknown>).totals === 'object') setMaintenanceTotals((data as Record<string, unknown>).totals as RecordItem); } }).catch((cause) => { if (!cancelled) setTabError(cause instanceof Error ? cause.message : String(cause)); }).finally(() => { if (!cancelled) setTabLoading(false); });
    return () => { cancelled = true; };
  }, [pointId, activeTab, tabRetry]);
  ...
  const header = <section className="pointIdentity" aria-label="Nokta kimliği ve durum">
    ...
    <div className="pointIdentityFacts">
      <span className={point.status === 'ACTIVE' ? 'pill active' : 'pill'}>{point.status === 'ACTIVE' ? 'Aktif' : point.status === 'PASSIVE' ? 'Pasif' : 'İptal'}</span>
      <span><strong>Bakım tipi:</strong> {maintenanceTypeLabel(point.maintenanceType)}</span>
      <span><strong>Geçerli teknisyen:</strong> {assignmentError ? <span role="alert">Geçerli teknisyen yüklenemedi · {assignmentError} <button className="ghost" onClick={() => setAssignmentRetry(value => value + 1)}>Geçerli teknisyeni yeniden dene</button></span> : effectiveAssignment ? effectiveAssignment.technician?.name ?? 'Atanmamış' : 'Yükleniyor…'}</span>
      <span className="pill" title={`Kaynak: ${point.locationSource ?? 'bilinmiyor'}`}><strong>Konum:</strong> {locationSummary(point)}</span>
    </div>
