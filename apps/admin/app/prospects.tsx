...
export default function Prospects() {
  ...
  async function load() {
    const [prospects, regionList] = await Promise.all([
  ...
  return <>
    ...
    <section className="panel">
      <div className="panelHeader"><div><h2>Potansiyel Müşteriler</h2><p>Sahadan gelen aday müşterileri incele ve uygun olanları gerçek bakım noktasına dönüştür.</p></div><button className="ghost" disabled={busy} onClick={() => void load()}>YENİLE</button></div>
      ...
      <div className="tableWrap"><table><thead><tr><th>Aday</th><th>Kaynak</th><th>SAP No</th><th>Konum</th><th>Oluşturan</th><th>Ziyaret</th><th></th></tr></thead><tbody>
        {items.length === 0 ? <tr><td colSpan={7}>Aktif potansiyel müşteri yok.</td></tr> : items.map((item) => <tr key={item.id}>
    ...
    {history ? <section className="panel">
      <div className="panelHeader"><div><h2>{history.prospect.name} · Geçmiş</h2><p>{history.prospect.sapNo ? `SAP ${history.prospect.sapNo}` : 'SAP No yok'}</p></div><button className="ghost" onClick={() => setHistory(null)}>KAPAT</button></div>
      <div className="tableWrap"><table><thead><tr><th>Tarih</th><th>Teknisyen</th><th>Amaç</th><th>Not</th></tr></thead><tbody>
        {history.prospect.visits.length === 0 ? <tr><td colSpan={4}>Henüz prospect ziyareti yok.</td></tr> : history.prospect.visits.map((visit) => <tr key={visit.id}><td>{new Date(visit.visitedAt).toLocaleString('tr-TR')}</td><td>{visit.technician.name}</td><td>{visit.purpose === 'SURVEY' ? 'Keşif' : 'Kurulum'}</td><td>{visit.note || '—'}</td></tr>)}
      </tbody></table></div>
      {history.audit.length ? <div className="muted">Audit: {history.audit.map((entry) => `${entry.action} · ${entry.actor?.name || '—'} · ${new Date(entry.createdAt).toLocaleString('tr-TR')}`).join(' | ')}</div> : null}
    </section> : null}
  </>;
}
