...
export default function CorporateApp() {
  ...
  function returnToTasksAfterNonMaintenance() {
    setNonMaintenanceCustomer(null);
    setNonMaintenanceNoCustomer(false);
    resetNavigation('TASKS');
  ...
  return <SafeAreaView edges={['top','bottom']} style={styles.safe}><StatusBar style="light" /><View style={styles.shell}>
    ...
    <ScrollView style={styles.scroll} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      ...
      {screen === 'CUSTOMERS' && <CustomersView customers={customers} search={customerSearch} setSearch={setCustomerSearch} open={openCustomer} openNonMaintenance={() => void openNonMaintenanceVisit()} />}
      ...
      {screen === 'NON_MAINTENANCE_VISIT_SAVED' && <NonMaintenanceSuccessView point={nonMaintenanceSuccessName} done={returnToTasksAfterNonMaintenance} />}
...
function NonMaintenanceForm({customer,noCustomer,customerName,setCustomerName,purpose,setPurpose,note,setNote,efesimImageBase64,visualExplanation,setVisualExplanation,chooseVisual,save,busy}:{customer:MyCustomer|null;noCustomer:boolean;customerName:string;setCustomerName:(value:string)=>void;purpose:NonMaintenanceVisitType;setPurpose:(value:NonMaintenanceVisitType)=>void;note:string;setNote:(value:string)=>void;efesimImageBase64:string;visualExplanation:string;setVisualExplanation:(value:string)=>void;chooseVisual:()=>void;save:()=>void;busy:boolean}) {
  return <><View style={styles.card}><Text style={styles.sectionLabel}>MÜŞTERİ</Text>{customer?<><Text style={styles.taskName}>{customer.name}</Text><Text style={styles.personMeta}>{customer.code}</Text></>:<TextInput accessibilityLabel="Müşteri adı" style={styles.input} value={customerName} onChangeText={setCustomerName} placeholder="Müşteri adı" maxLength={160}/>}</View><View style={styles.card}><Text style={styles.sectionLabel}>ZİYARET TÜRÜ</Text><VisitTypePicker purpose={purpose} setPurpose={setPurpose}/></View>{noCustomer?<View style={styles.card}><Text style={styles.sectionLabel}>EFESİM KANITI</Text><Text style={styles.help}>EFESİM ekran görüntüsü ekleyebilirsin. Görsel mevcut değilse açıklama ile kayıt engellenmeden devam eder.</Text><SecondaryButton title={efesimImageBase64?'EFESİM ekran görüntüsü seçildi':'EFESİM ekran görüntüsü seç'} icon="image" onPress={chooseVisual} disabled={busy}/><SecondaryButton title="Görsel yok, açıklamayla devam et" icon="edit-3" onPress={()=>setVisualExplanation(visualExplanation || 'EFESİM görseli mevcut değildi')} disabled={busy}/><TextInput accessibilityLabel="Görsel yok açıklaması" style={styles.input} value={visualExplanation} onChangeText={setVisualExplanation} placeholder="Görsel yoksa açıklama" multiline maxLength={500}/></View>:null}<View style={styles.card}><Text style={styles.sectionLabel}>NOT</Text><TextInput accessibilityLabel="Bakım dışı ziyaret notu" style={styles.input} value={note} onChangeText={setNote} placeholder="Ziyaret notu (isteğe bağlı)" multiline maxLength={300}/></View><PrimaryButton title="BAKIM DIŞI ZİYARETİ KAYDET" icon="check-circle" onPress={save} disabled={busy}/></>;
}

function VisitTypePicker({purpose,setPurpose}:{purpose:NonMaintenanceVisitType;setPurpose:(value:NonMaintenanceVisitType)=>void}) {
  const [open, setOpen] = useState(false);
  const selected = NON_MAINTENANCE_TYPES.find(item => item.value === purpose)?.label ?? 'Ziyaret türünü seç';
  const select = (value: NonMaintenanceVisitType) => {
    setPurpose(value);
    setOpen(false);
  };
  return <><TouchableOpacity accessibilityRole="button" accessibilityLabel="Ziyaret türü seç" accessibilityHint={selected} style={styles.selectButton} onPress={()=>setOpen(true)}><Text style={styles.selectValue}>{selected}</Text><Feather name="chevron-down" size={20} color={BLUE}/></TouchableOpacity><Modal transparent animationType="slide" visible={open} onRequestClose={()=>setOpen(false)}><View style={styles.dialogBackdrop}><SafeAreaView edges={['bottom']} style={styles.dialogSafe}><View style={styles.dialogSheet}><View style={styles.dialogHeader}><View><Text style={styles.dialogTitle}>Ziyaret türü</Text><Text style={styles.personMeta}>Bir işlem türü seç</Text></View><TouchableOpacity accessibilityRole="button" accessibilityLabel="Ziyaret türü listesini kapat" onPress={()=>setOpen(false)}><Feather name="x" size={24} color={TEXT}/></TouchableOpacity></View><ScrollView style={styles.visitTypeList}>{NON_MAINTENANCE_TYPES.map(item=><TouchableOpacity key={item.value} accessibilityRole="button" accessibilityState={{selected:purpose===item.value}} style={[styles.visitTypeOption,purpose===item.value&&styles.visitTypeOptionSelected]} onPress={()=>select(item.value)}><Text style={[styles.visitTypeOptionText,purpose===item.value&&styles.visitTypeOptionTextSelected]}>{item.label}</Text>{purpose===item.value?<Feather name="check" size={18} color="#fff"/>:null}</TouchableOpacity>)}</ScrollView><SecondaryButton title="VAZGEÇ" icon="x" onPress={()=>setOpen(false)}/></View></SafeAreaView></View></Modal></>;
}
...
const styles=StyleSheet.create({
  ...
  nonMaintenanceEntry:{backgroundColor:'#EAF4FC',borderWidth:1,borderColor:'#B9D5E8',borderRadius:14,padding:15,flexDirection:'row',alignItems:'center',justifyContent:'space-between'}, nonMaintenanceEntryTitle:{fontSize:15,fontWeight:'900',color:BLUE}, noCustomerChoice:{backgroundColor:'#FFF9EE',borderWidth:1,borderColor:'#F0D5AB',borderRadius:14,padding:14,flexDirection:'row',alignItems:'center'}, selectButton:{minHeight:50,borderWidth:1,borderColor:'#BCD0DF',backgroundColor:'#F8FBFD',borderRadius:10,paddingHorizontal:14,flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:9}, selectValue:{flex:1,fontSize:15,fontWeight:'900',color:BLUE}, visitTypeList:{maxHeight:390}, visitTypeOption:{minHeight:48,borderWidth:1,borderColor:'#BCD0DF',backgroundColor:'#F8FBFD',borderRadius:10,paddingHorizontal:14,marginBottom:8,flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:10}, visitTypeOptionSelected:{backgroundColor:BLUE,borderColor:BLUE}, visitTypeOptionText:{fontSize:14,fontWeight:'900',color:BLUE}, visitTypeOptionTextSelected:{color:'#fff'},
