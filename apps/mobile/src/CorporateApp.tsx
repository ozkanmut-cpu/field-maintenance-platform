...
export default function CorporateApp() {
  ...
  return <SafeAreaView edges={['top','bottom']} style={styles.safe}><StatusBar style="light" /><View style={styles.shell}>
    ...
    <ScrollView style={styles.scroll} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      ...
      {screen === 'NEARBY' && (nearbyError ? <Empty text={nearbyError} /> : <NearbyScreen items={nearbyItems} origin={deviceLocation} loading={nearbyLoading} refresh={() => void loadNearby()} directions={openNearbyDirections} />)}
    ...
    <View style={styles.nav}><Nav label="İşler" icon="clipboard" active={screen === 'TASKS' || screen === 'SUCCESS' || screen === 'EQUIPMENT_CONFIRM'} onPress={() => setScreen('TASKS')} /><Nav label="Yakınımdakiler" icon="map-pin" active={screen === 'NEARBY'} onPress={() => void openNearby()} /><Nav label="Müşterilerim" icon="users" active={screen === 'CUSTOMERS' || screen === 'CUSTOMER'} onPress={() => void openCustomers()} /><Nav label="Yardım Et" icon="user-plus" active={screen === 'HELP'} onPress={() => void openHelp()} /><Nav label="Geçmiş" icon="clock" active={screen === 'HISTORY'} onPress={() => void openHistory()} /></View>
  </View></SafeAreaView>;
}

function SearchBox({value,onChange,placeholder}:{value:string;onChange:(v:string)=>void;placeholder:string}) { return <View style={styles.searchBox}><Feather name="search" size={18} color="#667989" /><TextInput style={styles.searchInput} value={value} onChangeText={onChange} placeholder={placeholder} placeholderTextColor="#8795A1" autoCorrect={false} returnKeyType="search" />{value?<TouchableOpacity onPress={()=>onChange('')}><Feather name="x-circle" size={18} color="#8795A1" /></TouchableOpacity>:null}</View>; }
...
function Task({task,busy,deviceLocation,onDirections,onAttempt,onComplete}:{task:DueTask;busy:boolean;deviceLocation?:{latitude:number;longitude:number}|null;onDirections:(t:DueTask)=>void;onAttempt:(t:DueTask)=>void;onComplete:(t:DueTask)=>void}) { const late=task.priority==='OVERDUE'; const distance=deviceLocation&&task.latitude!=null&&task.longitude!=null?distanceMetersStatic(deviceLocation.latitude,deviceLocation.longitude,task.latitude,task.longitude):null; return <View style={styles.task}><View style={styles.taskTop}><View style={[styles.statusDot,late?styles.redDot:styles.orangeDot]}/><Text style={late?styles.lateText:styles.currentText}>{late?'GECİKMİŞ':'BU DÖNEM'}{task.overduePeriods>0?` · ${task.overduePeriods} dönem`:''}</Text></View><Text style={styles.taskName}>{task.pointName}</Text><Text style={styles.taskMeta}>{task.pointCode} · {task.regionName}{distance!=null?` · ${formatDistance(distance)}`:''}</Text><TouchableOpacity style={styles.routeButton} onPress={()=>onDirections(task)} disabled={busy}><View style={styles.buttonContent}><Feather name="navigation" size={16} color={BLUE} /><Text style={styles.routeText}>YOL TARİFİ</Text></View></TouchableOpacity><PrimaryButton title="BAKIM YAPILDI" onPress={()=>onComplete(task)} disabled={busy}/><TouchableOpacity style={styles.failButton} onPress={()=>onAttempt(task)} disabled={busy}><View style={styles.buttonContent}><Feather name="alert-circle" size={16} color="#B7372F" /><Text style={styles.failText}>Bakım yapılamadı</Text></View></TouchableOpacity></View>; }
...
function CustomersView({customers,search,setSearch,open}:{customers:MyCustomer[];search:string;setSearch:(v:string)=>void;open:(c:MyCustomer)=>void}) { const visible=customers.filter(c=>matchesSearch(search, [c.name, c.code, c.region?.name ?? '', c.address ?? '', ...(c.aliases ?? [])])); return <View style={styles.card}><Text style={styles.cardTitle}>Müşterilerim</Text><Text style={styles.help}>Bakım zamanı gelmeden de müşteri ekipman bilgilerini buradan tamamlayabilirsin.</Text><SearchBox value={search} onChange={setSearch} placeholder="Müşteri adı, kod, bölge, adres veya eski ad ara" />{customers.length===0?<Empty text="Atanmış aktif müşteri yok."/>:visible.length===0?<Empty text="Aramayla eşleşen müşteri yok"/>:visible.map(c=><TouchableOpacity key={c.id} style={styles.personRow} onPress={()=>open(c)}><View style={styles.personText}><Text style={styles.personName}>{c.name}</Text><Text style={styles.personMeta}>{c.code}{c.region?.name?` · ${c.region.name}`:''}</Text><Text style={c.equipmentComplete?styles.okText:styles.warningText}>{c.equipmentComplete?'Ekipman bilgisi tamam':'Ekipman bilgisi eksik'}</Text></View><Text style={styles.chevron}>›</Text></TouchableOpacity>)}</View>; }
...
const styles=StyleSheet.create({
  safe:{flex:1,backgroundColor:DARK_BLUE}, shell:{flex:1,backgroundColor:LIGHT}, scroll:{flex:1}, container:{padding:18,paddingBottom:30,gap:14}, center:{flex:1,alignItems:'center',justifyContent:'center',gap:12,backgroundColor:LIGHT},
  header:{backgroundColor:DARK_BLUE,paddingHorizontal:22,paddingTop:18,paddingBottom:20,flexDirection:'row',justifyContent:'space-between',alignItems:'center',borderBottomWidth:1,borderBottomColor:'rgba(255,255,255,.08)'}, eyebrow:{color:'#BFD8EC',fontSize:11,fontWeight:'900',letterSpacing:1.4}, headerTitle:{color:'#fff',fontSize:25,fontWeight:'900',marginTop:2}, headerUser:{color:'#E6F0F7',fontSize:13,marginTop:2}, logoutBtn:{borderWidth:1,borderColor:'rgba(255,255,255,.24)',backgroundColor:'rgba(255,255,255,.06)',paddingHorizontal:13,paddingVertical:9,borderRadius:10}, logoutText:{color:'#fff',fontSize:11,fontWeight:'900'},
  loginSafe:{flex:1,backgroundColor:DARK_BLUE}, loginHero:{flex:1.05,alignItems:'center',justifyContent:'center',gap:9,padding:24}, logoMark:{width:72,height:72,borderRadius:22,backgroundColor:BLUE,alignItems:'center',justifyContent:'center',borderWidth:1,borderColor:'rgba(255,255,255,.18)'}, logoText:{color:'#fff',fontSize:43,fontWeight:'900',fontStyle:'italic'}, loginTitle:{color:'#fff',fontSize:32,fontWeight:'900'}, loginSub:{color:'#C8DDED',fontSize:14}, loginForm:{backgroundColor:LIGHT,padding:24,gap:12}, loginInput:{backgroundColor:'#fff',borderWidth:1,borderColor:'#D4DEE6',borderRadius:12,padding:14,fontSize:16,color:'#172431'},
  ...
  sectionHead:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginTop:3}, sectionTitle:{fontSize:18,fontWeight:'900',color:'#182633'}, refresh:{fontSize:11,fontWeight:'900',color:BLUE}, searchBox:{backgroundColor:'#fff',borderWidth:1,borderColor:'#D8E2E9',borderRadius:12,minHeight:46,paddingHorizontal:12,flexDirection:'row',alignItems:'center',gap:9}, searchInput:{flex:1,fontSize:15,color:'#1A2733',paddingVertical:10}, card:{backgroundColor:'#fff',borderRadius:15,padding:16,gap:12}, cardTitle:{fontSize:19,fontWeight:'900',color:'#182633'}, help:{fontSize:14,lineHeight:20,color:'#637485'},
  task:{backgroundColor:'#fff',borderRadius:16,padding:17,gap:10,borderWidth:1,borderColor:'#E2E9EF',shadowColor:'#173349',shadowOpacity:.035,shadowRadius:10,elevation:1}, taskTop:{flexDirection:'row',alignItems:'center',gap:7}, statusDot:{width:9,height:9,borderRadius:5}, redDot:{backgroundColor:RED}, orangeDot:{backgroundColor:ORANGE}, lateText:{color:RED,fontSize:12,fontWeight:'900'}, currentText:{color:ORANGE,fontSize:12,fontWeight:'900'}, taskName:{fontSize:19,fontWeight:'900',color:'#1A2733'}, taskMeta:{fontSize:13,color:'#667989'}, routeButton:{borderWidth:1,borderColor:'#BCD0DF',backgroundColor:'#F8FBFD',borderRadius:10,padding:11,alignItems:'center'}, routeText:{color:BLUE,fontSize:12,fontWeight:'900'}, primary:{backgroundColor:'#0877D1',borderRadius:11,paddingVertical:14,paddingHorizontal:14,alignItems:'center',minHeight:48,justifyContent:'center'}, primaryText:{color:'#fff',fontSize:14,fontWeight:'900'}, failButton:{borderWidth:1,borderColor:'#E2B7B4',borderRadius:10,paddingVertical:11,alignItems:'center'}, failText:{color:'#B7372F',fontSize:13,fontWeight:'800'}, disabled:{opacity:.45},
  ...
  nav:{height:70,backgroundColor:'#fff',borderTopWidth:1,borderTopColor:'#DDE5EB',flexDirection:'row',paddingTop:5,paddingBottom:5,shadowColor:'#173349',shadowOpacity:.06,shadowRadius:10,elevation:8}, navItem:{flex:1,alignItems:'center',justifyContent:'center',gap:4}, navSymbol:{fontSize:20,color:'#80909D'}, navLabel:{fontSize:10,color:'#80909D',fontWeight:'800'}, navActive:{color:'#0877D1',fontWeight:'900'},
