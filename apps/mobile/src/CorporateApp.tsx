...
import {
  ...
  TechnicianDashboard, technicianHistory, TechnicianHistoryItem,
} from './api';

type Screen = 'TASKS' | 'HELP' | 'NEW' | 'HISTORY' | 'EFESIM_RESULT' | 'PROSPECT' | 'VISIT_SAVED' | 'SUCCESS';
...
export default function CorporateApp() {
  ...
  const title = screen === 'HELP' ? 'Yardım Et' : screen === 'NEW' || screen === 'EFESIM_RESULT' || screen === 'PROSPECT' || screen === 'VISIT_SAVED' ? 'Yeni Nokta' : screen === 'HISTORY' ? 'Geçmiş' : 'İşler';
  return <SafeAreaView style={styles.safe}><StatusBar style="light" /><View style={styles.shell}>
    ...
    <ScrollView style={styles.scroll} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      {screen === 'TASKS' && <TaskList dashboard={dashboard} busy={busy} refresh={() => void loadTasks()} onDirections={openDirections} onAttempt={task => attemptReason(task)} onComplete={task => void completeTask(task)} />}
      {screen === 'HELP' && <HelpView people={helpPeople} dashboard={helpDashboard} busy={busy} select={selectHelper} change={() => setHelpDashboard(null)} onDirections={openDirections} onAttempt={task => attemptReason(task, helpDashboard?.technician.id)} onComplete={task => void completeTask(task, helpDashboard?.technician.id)} />}
    ...
    <View style={styles.nav}><Nav label="İşler" symbol="▣" active={screen === 'TASKS' || screen === 'SUCCESS'} onPress={() => setScreen('TASKS')} /><Nav label="Yardım Et" symbol="♧" active={screen === 'HELP'} onPress={() => void openHelp()} /><Nav label="Yeni Nokta" symbol="⊕" active={['NEW','EFESIM_RESULT','PROSPECT','VISIT_SAVED'].includes(screen)} onPress={() => setScreen('NEW')} /><Nav label="Geçmiş" symbol="↺" active={screen === 'HISTORY'} onPress={() => void openHistory()} /></View>
  </View></SafeAreaView>;
}
