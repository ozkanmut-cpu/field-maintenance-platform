...
export function TasksScreen({ dashboard, loading, error, search, filter, deviceLocation, assistanceTargets, assistedForTechnicianId, assistanceSelectorVisible, assistanceLoading, assistanceError, onSearchChange, onFilterChange, onRefresh, onOpenTask, onOpenAssistance, onSelectAssistance, onChangeAssistance, onExitAssistance, onRetryAssistance }: TasksScreenProps) {
  ...
  return <View style={styles.screen}>
    {dashboard && <View style={styles.summary}>
      ...
      <TouchableOpacity accessibilityRole="button" accessibilityLabel="Yardım modu seç" style={styles.helpButton} onPress={onOpenAssistance} disabled={assistanceLoading}>
        <Feather name="users" size={17} color="#075A96" /><Text style={styles.helpButtonText}>YARDIM ET</Text>
      </TouchableOpacity>
    </View>}
    ...
    {loading && !dashboard ? <TaskSkeleton /> : error ? <RetryState error={error} onRetry={onRefresh} /> : dashboard && tasks.length === 0 ? <EmptyState searched={Boolean(search.trim()) || filter !== 'ALL'} /> : tasks.map(task => <TaskCard key={task.pointId} task={task} deviceLocation={deviceLocation} onPress={onOpenTask} />)}
  </View>;
}
...
const styles = StyleSheet.create({
  screen: { padding: 16, gap: 12 }, summary: { backgroundColor: '#fff', borderRadius: 14, padding: 15, borderWidth: 1, borderColor: '#DDE5EB', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, summaryEyebrow: { fontSize: 10, color: '#5C7080', fontWeight: '900', letterSpacing: .8 }, summaryTitle: { color: '#173349', fontSize: 19, fontWeight: '900', marginTop: 2 }, helpButton: { minHeight: 48, paddingHorizontal: 11, borderRadius: 10, borderWidth: 1, borderColor: '#C8D8E4', flexDirection: 'row', gap: 6, alignItems: 'center', justifyContent: 'center' }, helpButtonText: { color: '#075A96', fontSize: 11, fontWeight: '900' },
