...
export function AttemptScreen({ task, submitting, error, onBack, onIntentChange, onSubmit }: AttemptScreenProps) {
  ...
  return <View style={styles.screen}>
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>Not <Text style={styles.optional}>(isteğe bağlı)</Text></Text>
        <TextInput accessibilityLabel="Bakım yapılamama notu" style={styles.noteInput} value={note} onChangeText={value => { onIntentChange(); setNote(value); }} editable={!submitting} multiline maxLength={500} placeholder="Kısa açıklama ekle" placeholderTextColor="#8795A1" textAlignVertical="top" />
      </View>
...
const styles = StyleSheet.create({
  ...
  heroCard: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#DDE5EB', borderRadius: 14, padding: 16, gap: 6 }, eyebrow: { color: '#A96308', fontSize: 10, fontWeight: '900', letterSpacing: .7 }, title: { color: '#173349', fontSize: 22, fontWeight: '900' }, meta: { color: '#70818E', fontSize: 13, fontWeight: '700' },
  ...
  optional: { color: '#70818E', fontSize: 13, fontWeight: '700' }, noteInput: { minHeight: 76, borderWidth: 1, borderColor: '#C8D8E4', borderRadius: 10, padding: 12, color: '#173349', fontSize: 14, lineHeight: 20 },
